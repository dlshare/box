package com.box.myview.MySwitchButton;
...
/**
 * Created by zhangbeibei on 2017/4/6
 */
...
public class MySwitchButton extends View implements View.OnClickListener {
    ...
    public MySwitchButton(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.getTheme().obtainStyledAttributes(attrs, R.styleable.MySwitchButton, defStyleAttr, R.style.def_switchbutton);
        int indexCount = typedArray.getIndexCount();
        for (int i = 0; i < indexCount; i++) {
            int attr = typedArray.getIndex(i);
            if (attr == R.styleable.MySwitchButton_switch_bg_color) {
                switchViewBgColor = typedArray.getColor(attr, Color.BLACK);
                continue;
            }
            if (attr == R.styleable.MySwitchButton_switch_ball_color) {
    ...
    public interface OnCheckedChangeListener {
        void onCheckedChanged(MySwitchButton buttonView, boolean isChecked);
    }
